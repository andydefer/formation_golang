...
**Contenu actuel :** Modules 1 à 8 (fondamentaux, approfondissement, modularité, gestion d'erreurs et interfaces)
**À venir :** Modules 9 à 16 (avancé à professionnel)
...
1. **Par module** : Suivez l'ordre recommandé (Module 1 → 8, puis la suite)
...
### Exemple de réalisation (Module 8)
...
```go
// main.go avec polymorphisme
func EffectuerPaiement(p paiement.Payeur, montant float64) {
    err := p.Payer(montant)
    if err != nil {
        fmt.Printf("❌ %s: %v\n", p.GetNom(), err)
    } else {
        fmt.Printf("✅ %s: paiement de %.2f€ effectué\n", p.GetNom(), montant)
    }
}
...
func AfficherInfos(p interface{}) {
    ...
    case *paiement.CarteBancaire:
        fmt.Printf("[CB] %s - Solde: %.2f€\n", v.Titulaire, v.Solde)
    case *paiement.PayPal:
        fmt.Printf("[PayPal] %s - Solde: %.2f€\n", v.Email, v.Solde)
    }
}
```
...
Semaine 8-9 : Modules 9-10 (Concurrence base à Fichiers)
Semaine 10-11: Modules 11-16 (Concurrence avancée à Organisation)
